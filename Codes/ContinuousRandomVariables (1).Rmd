...
x = seq(138,202,0.1)
f_X = function(x){
  return(exp(-(x-170)^2/(2*(8^2)))/sqrt(2*pi*64)) #mean = 170 , SD = 8
}
f_Y = function(x){
  return(exp(-(x-170)^2/(2*(4^2)))/sqrt(2*pi*16))
}
#y1 = f_X(x) 
y1 = dnorm(x,170,8)  #How much  density available at input value x at normally distributed values of mean 170 and SD 8
...
x = 165
dnorm(x,mu,sigma)
pnorm(x,mu,sigma) #area of left of 165 in density curve
pnorm(173,mu,sigma) - pnorm(169,mu,sigma) #area of left of 173 in density curve - #area of left of 169 in density curve
pnorm(173,mu,sigma) # prob till 173 - at most 173
pnorm(173,mu,sigma,lower.tail= FALSE) # prob that the random students height is greater then 173 - right side of 173 in the histogram - at least 175 
```
...
```{r}
sum(simulatedData_Male)/ nsimulations #same as mean(simulatedData_Male)
mean((simulatedData_Male - mean(simulatedData_Male))^2)  #almost same as var(simulatedData_Male)
